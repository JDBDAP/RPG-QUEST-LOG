...
}`Practice session scoring. XP scale: 6000 XP = 1 level.\nBaseline (time only): ${baseXp} XP (${f.dur} min).\nType: ${ptype?.label}, Skills: ${skNames}\nJournal: "${f.note}"\nScore for QUALITY and DEPTH — a profound session justifies many levels. Routine/mechanical = baseline. Focused/deep = 1.5-3×. Breakthrough insight = 3-10×. Never cap at 100.\nJSON only: {"xp":number,"reason":"12 words max"}`
          })
        });
        const data=await res.json();
        const _ptxt=data.choices?.[0]?.message?.content||"{}";
...
}`Practice session preview. XP scale: 6000 XP = 1 level.
Baseline (time only): ${baseXp} XP.
Type: ${ptype.label}, ${f.dur}min, Skills: ${skNames}${f.note.trim()?`
Journal: "${f.note}"`:""}.
Score for quality/depth: routine=~baseline, focused=1.5-3×, breakthrough=3-10×. Never cap at 100.
JSON only, no markdown: {"xp": NUMBER, "reason": "max 15 words"}`})});
      const data=await res.json();
      const txt=data.choices?.[0]?.message?.content||"";
      const m=txt.match(/\{[\s\S]*\}/);
      if(m) setXpPreview(JSON.parse(m[0]));
